...
<code>function areYouHere(arr1, arr2) {
...
<code>function doubleArrayValues(array) {
...
<code>function naiveSearch(array, item) {
    for (let i = 0; i < array.length; i++) {
        if (array[i] === item) {
            return i;
        }
...
<code>function efficientSearch(array, item) {
    let minIndex = 0;
